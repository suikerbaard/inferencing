"""
SQ3: distance bands (train and evaluate)

For each radius R in 0, 15, 35 and 60 km and each held out station: every
station within R km of the target is removed from the fold, the neighbour
features are rebuilt from the reduced network, a fresh encoder + decoder pair
is trained (or loaded on a rerun) and the held out station's 2024 hours are
predicted. 4 radii x 31 stations = 124 model pairs.

Saved to results/models/ : sq3_r<R>km_<station>_{stdgi,decoder}.pt
Saved to results/tables/ (per radius, same names as the XGBoost notebook):
    sq3_r<R>km_per_station_metrics.csv, sq3_r<R>km_predictions.csv,
    sq3_r<R>km_hourly_metrics.csv
    sq3_summary.csv               mean and median metrics per radius
    sq3_wilcoxon_vs_0km.csv       paired Wilcoxon of each radius against 0 km (BH)

The 0 km setting is the intact network and matches the plain LOSO of script 01.
Figures are made by 05_agnn_evaluation_and_figures.ipynb from these CSVs.
"""
...
# per fold models are archived here (loaded on a rerun so evaluation does
# not retrain); all evaluation result CSVs go to results/tables
...
# the exclusion radii of the thesis distance band experiment
RADII = [0, 15, 35, 60]

pd.set_option("display.max_columns", None)
print("device:", DEVICE, "| radii:", RADII, "km")


# Feature groups and the AGNN feature roles 
...
def build_fold_data(held_out, radius_km):
    #radius_km does the network thinning
...
def run_fold(fold, stdgi_path, decoder_path):
    ...
    if resumed:
        ...
        scratch_e = os.path.join(MODELS_DIR, "_scratch_sq3_stdgi.pt")
        scratch_d = os.path.join(MODELS_DIR, "_scratch_sq3_decoder.pt")
...
# Train (or load) each station/radius, evaluate, save the result tables
...
for radius in RADII:
    ...
    for k, held_out in enumerate(STATION_IDS):
        tag = "sq3_r" + str(radius) + "km_" + held_out
        stdgi_path = os.path.join(MODELS_DIR, tag + "_stdgi.pt")
        decoder_path = os.path.join(MODELS_DIR, tag + "_decoder.pt")

        fold = build_fold_data(held_out, radius_km=radius)
        ...
        m["radius_km"] = radius
        m["n_removed"] = len(fold["removed"])
        m["n_available"] = len(STATION_IDS) - 1 - len(fold["removed"])
        per_rows.append(m)

        status = "loaded " if resumed else "trained"
        minutes = (time.time() - t0) / 60.0
        print("  [%2d/%d] %s %s: RMSE %.3f | removed %2d | %.1f min"
              % (k + 1, len(STATION_IDS), status, held_out, m["RMSE"], len(fold["removed"]), minutes))
...
# paired Wilcoxon of every radius against the intact network, BH corrected
...
print()
print("saved the sq3 per radius tables, sq3_summary.csv and sq3_wilcoxon_vs_0km.csv")


# remove the scratch checkpoints, only the final per fold files matter
for scratch in ["_scratch_sq3_stdgi.pt", "_scratch_sq3_decoder.pt"]:
