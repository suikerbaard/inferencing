"""
SQ4: seasonal performance 

Trains the same full LOSO model as script 01  (standalone), 
evaluates it on 2024, and splits the predictions by meteorological season 
(Winter Dec/Jan/Feb, Spring Mar/Apr/May, Summer Jun/Jul/Aug, Autumn Sep/Oct/Nov).

Saved to results/models/ : sq4_<station>_{stdgi,decoder}.pt (loaded on a rerun)
Saved to results/tables/ :
    sq4_per_station_metrics.csv, sq4_predictions.csv, sq4_hourly_metrics.csv
    sq4_per_station_season_metrics.csv    RMSE, MAE, R2 per station per season
    sq4_seasonal_table.csv                mean concentration, RMSE, nRMSE, MAE, R2 per season
    sq4_season_pairwise_wilcoxon.csv      pairwise season tests (BH)
...
def run_fold(fold, stdgi_path, decoder_path):
    ...
    if resumed:
        ...
        scratch_e = os.path.join(MODELS_DIR, "_scratch_sq4_stdgi.pt")
        scratch_d = os.path.join(MODELS_DIR, "_scratch_sq4_decoder.pt")
...
# Train (or load) each fold, evaluate on 2024, split by season
...
for k, held_out in enumerate(STATION_IDS):
    stdgi_path = os.path.join(MODELS_DIR, "sq4_" + held_out + "_stdgi.pt")
    decoder_path = os.path.join(MODELS_DIR, "sq4_" + held_out + "_decoder.pt")
    ...
    pd.concat(pred_parts, ignore_index=True).to_csv(
        os.path.join(OUT_TABLES, "sq4_predictions.csv"), index=False, float_format="%.4f")
...
# pairwise Wilcoxon between the seasons on per station seasonal RMSE, BH corrected
...
print()
print("saved the sq4 per station, seasonal and season test tables")


# remove the scratch checkpoints, only the final per fold files matter
for scratch in ["_scratch_sq4_stdgi.pt", "_scratch_sq4_decoder.pt"]:
